...
def parse_args():
    ...
    parser.add_argument('--snapshot', default=20, type=int, help='save models for every #snapshot epochs (default: 20)')
    ...
    parser.add_argument('--lamda', '--weight_L1_norm', default=0.2, type=float, metavar='N', help='scale of L1 Norm')
    ...
    if args.evaluate:
        ...
        for i, action in enumerate(action_filter):
            ...
            error_eval_p1[i], error_body[i], error_face[i], error_hand[i], error_face_aligned[i], error_hand_aligned[
                i] = \
                evaluate(valid_loader, model_pos, device)
...
def train(data_loader, model_pos, model_post_refine, lamda, criterion, criterionL1, optimizer, device, lr_init, lr_now,
    ...
    for i, (targets_3d, inputs_2d, _, batch_cam) in enumerate(data_loader):
        ...
        # loss_3d_hand = ((1 - lamda) * criterion(hand_3d, targets_3d[:, 91:112, :]) + lamda *
        #                 criterionL1(hand_3d, targets_3d[:, 91:112, :]) + loss_post_refine)
        ...
        loss_3d_pos = loss_3d_body + loss_3d_face + loss_3d_left_hand + loss_3d_right_hand
...
def evaluate(data_loader, model_pos, device):
    ...
    for i, (targets_3d, inputs_2d, _, _) in enumerate(data_loader):
        ...
        # hand_3d_aligned = torch.cat((hand_3d[:, :21, :] - hand_3d[:, :1, :], hand_3d[:, 21:, :] - hand_3d[:, 21:22, :]),
        #                             dim=1)
